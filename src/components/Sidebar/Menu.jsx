...
import { gateremaps, contrbl, location, timeline, data, recents, print, share, lang } from "../../assets/images/Images";
...
const Menu = ({ openMenu, setOpenMenu }) => {
  ...
  return (
    <div
      className={classNames("px-5 flex flex-row absolute top-0 z-50 bg-[#fff] md:w-[23.5%] text-[#202124] text-xs h-[100%] overflow-y-scroll overflow-x-hidden transition-transform duration-300 transform", {
      ...
      <div className="w-[100%] ">
        <div className="flex pt-3 w-[100%] justify-between items-center">
          <a href="" className="cursor-pointer">
            <img className=" w-auto h-4 md:h-6" src={gateremaps} alt="gateremaps" />
          </a>
          <IoMdClose onClick={handleCloseMenu} className="text-[#616161] text-2xl cursor-pointer" />
        </div>
        <div className=" flex justify-between items-center my-3">
          ...
          >
            <span className={!isSelected ? "w-5 h-5 absolute bg-[#ffffff] rounded-full shadow shadow-[#898a8d] border border-[#f7f7f7]" : "w-5 h-5 absolute bg-[#1A73E8] rounded-full shadow shadow-[#898a8d] "} />
          </div>
        </div>
        <hr className="w-[110%]" />
        <div className="my-3 px-1">
          {menuItemsOne.map((item) => {
            return (
              <div key={item.id} className="flex gap-6 items-center text-[#616161] hover:text-[#1a73e8] my-3 cursor-pointer grayscale hover:grayscale-0">
                ...
                {item.id !== 1 && item.id !== 2 && <img src={item.icon} className=" w-6" alt="icon" />}
        ...
        <div className="my-3 px-1">
          {menuItemsTwo.map((item) => {
            return (
              <div key={item.id} className="flex gap-6 items-center text-[#494949] hover:text-[#1a73e8] my-3 cursor-pointer grayscale hover:grayscale-0">
                {/* <FaRegBookmark className="text-[#70757a] text-xl" /> */}
                {item.icon !== null && <img src={item.icon} className="w-6" alt="icon" />}
        ...
        <div className="my-3 px-1">
          {menuItemsThree.map((item) => {
            return (
              <div key={item.id} className="flex gap-6 items-center text-[#494949] hover:text-[#1a73e8] my-3 cursor-pointer">
                <p className="text-xs my-[1px]">{item.name}</p>
              </div>
            );
          })}
